...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
            ...
            if uncertainty is not None:
                if 'cls' in uncertainty.keys():
                    for k, v in uncertainty['cls'].items():
                        uncertainty['cls'][k] = v[keep_idxs]
                    mlvl_unc.append(uncertainty)
            else:
                mlvl_unc = None

        return self._bbox_post_process(mlvl_scores, mlvl_labels, mlvl_bboxes,
                                       img_meta['scale_factor'], cfg, rescale,
                                       with_nms, mlvl_score_factors, mlvl_unc,
                                       **kwargs)
    ...
    def _bbox_post_process(self,
        ...
        if mlvl_unc is not None:
            mlvl_uncertainties = []
            for k in mlvl_unc[0].keys():
                for k1 in  mlvl_unc[0][k].keys():
                    unc = torch.cat([level[k][k1] for level in mlvl_unc])
                    mlvl_uncertainties.append(unc)
        ...
        if with_nms:
            ...
            det_bboxes, keep_idxs = batched_nms(mlvl_bboxes, mlvl_scores,
                                                mlvl_labels, cfg.nms)
            if mlvl_unc is not None:
                for unc in mlvl_uncertainties:
                    det_bboxes = torch.cat([det_bboxes, unc[keep_idxs].unsqueeze(dim=1)], dim =1)
