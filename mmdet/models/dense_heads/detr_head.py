...
class DETRHead(AnchorFreeHead):
    ...
    def _get_bboxes_single(self,
        ...
        if "uncertainty" in self.test_cfg:
            uncertainty = Uncertainty.get_uncertainties(Uncertainty, self.test_cfg.uncertainty, logits=cls_score)
        else:
            uncertainty = None
        ...
        if uncertainty is not None:
            if 'cls' in uncertainty.keys():
                for k, v in uncertainty['cls'].items():
                    det_bboxes = torch.cat([det_bboxes, v[bbox_index].unsqueeze(dim=1)], dim =1)
