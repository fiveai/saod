...
def main():
    ...
    for sev_i, corruption_severity in enumerate(severities):
        ...
        if rank == 0 and args.eval and args.out:
            file_name = args.out[:-4] + '_severity_' + str(corruption_severity) + args.out[-4:]
            result_files = dataset.results2json(outputs, file_name)
            if test_data_cfg['ann_file'] != 'data/saod/annotations/sinobj110kood.json':
                eval_results = coco_eval(result_files, args.eval, dataset.coco)
